...
df['date'] = pd.to_datetime(df['date'])
df = df.groupby([df['date'].dt.date,df['financials'],df['ticker']])['value'].sum().unstack()

export_excel = df.to_excel(r'C:/Users/Jacob Steenhuysen/Downloads/Data Center Balance Sheet Histories.xlsx', sheet_name="Sheet1", index= True)
